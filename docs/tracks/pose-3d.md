...
`indices` / `kept`) を `mutable` メンバスクラッチへ移し `clear()` 再利用。Halpe26 × 2cam で
1 三角測量フレームあたり ~100 個の小ヒープ確保を除去。`triangulate()` は central loop 単一
スレッドからのみ呼ばれるため `mutable` 共有スクラッチは競合なし。出力はビット同一
(`test_triangulator` golden 通過)。微最適化のため design doc なし (changelog のみ)。
