...
| pose-3d raw 3D source | `ctest --test-dir cpp/build --output-on-failure -R 'main_config\|raw_3d_source\|snapshot_floor_stats'` + `./cpp/build/main --help` + 実機 `/ws3d` JSONL と `dump_keypoints_3d --no-kalman --no-ik --no-floor-contact-stability` 比較 | `--no-3d-postprocess` / `three_d.no_3d_postprocess: true` 時、`persons_3d` は triangulator 出力と一致し、Kalman / IK / floor-contact の state を更新しない。`stats.raw_3d_source=true`、`kalman_enabled=false`、`ik_enabled=false`、`floor_stability_enabled=false` が fresh / sync-miss / idle で確認できる。設定 round-trip は個別 stage 値を失わず、既定 mode の VMT/WebUI 挙動は不変。設計: [`design/pose-3d-raw-3d-source.md`](design/pose-3d-raw-3d-source.md) |
