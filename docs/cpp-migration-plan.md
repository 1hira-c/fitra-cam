...
| pose-3d `fitra_pose_gate_v1` | `cmake --build cpp/build && ctest --test-dir cpp/build --output-on-failure -R 'pose_gate'` + 実機 `--enable-3d --keypoint-format=halpe26` で `WS /ws/pose-gate` を JSONL 保存 | `test_pose_gate` が正常、関節欠損、再接続、人物切替、epoch変更、Halpe26以外、multi-person拒否、quaternion禁止を pass。実機は I/Ski 用8関節が `Fresh`、欠損/人物切替/再接続/再較正で Unavailable 境界と track ID/epoch 変更が記録され、`content_mono_ns` が単調時計であること。fixture: [`samples/fitra_pose_gate_v1`](samples/fitra_pose_gate_v1)、設計: [`design/pose-3d-fitra-pose-gate-v1.md`](design/pose-3d-fitra-pose-gate-v1.md) |
