...
| pose-3d raw 3D source | `ctest --test-dir cpp/build --output-on-failure -R 'main_config\|raw_3d_source\|snapshot_floor_stats\|vmt_protocol'` + `./cpp/build/main --help` + 実機 `/ws3d` JSONL と `dump_keypoints_3d --no-kalman --no-ik --no-floor-contact-stability` 比較 | `--no-3d-postprocess` / `three_d.no_3d_postprocess: true` 時、run mode の `persons_3d` は triangulator 出力と一致し、Kalman / IK / floor-contact の state を更新しない。`stats.raw_3d_source=true`、`kalman_enabled=false`、`ik_enabled=false`、`floor_stability_enabled=false` が fresh / sync-miss / idle で確認できる。raw + VMT は profile / IK lock 未指定でも `valid_joints>0` の fresh data だけ bundle を送信し、起動直後 / sync-miss / idle の空 snapshot は送信しない。通常 VMT は従来の `ik_locked` gate を維持する。CalibSubject は共有 YAML の raw umbrella を無効化し、post-IK drift gate を維持する。設定 round-trip は個別 stage 値を失わず、既定 mode の WebUI/VMT 挙動は不変。設計: [`design/pose-3d-raw-3d-source.md`](design/pose-3d-raw-3d-source.md) |
