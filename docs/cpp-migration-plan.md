...
>
> **2026-08-14 更新 (D50 fusion producer)**: 既存 `fitra_pose_gate_v1` は固定したまま、
> `triangulator->triangulate()` 直後のraw観測と、Kalman/IK後・floor/root出力補正前の
> filtered位置を同じcapture lineageへ束ねる additiveな `FusionPoseBus` を生成する。
> capture threadはV4L2 kernel monotonic timestampとSOE/EOF意味論をDQBUF観測時刻とは別に保持し、
> `Frame`→`DecodedFrame`→同期`CameraSnapshot`へ伝播する。新 `fitra_fusion_pose_v1` は
> 10関節（既存8 + 左右shoulder）のraw最終inlier品質、同captureの`filtered_position_m`、capture区間、source publish monotonic時刻、continuityを公開し、通常poseは
> latest-only、境界は有界FIFO、欠落時は`ContinuityReset`とする。run/calib-subjectの両composition
> rootから`GET /api/fusion-pose` / `WS /ws/fusion-pose`へ配線し、同WSでnumeric clock-syncを扱う。
> `observed_this_frame`はraw品質条件だけで決まり、Hipsは同capture左右hip midpointを保守合成する。
> 詳細は [`design/pose-3d-fusion-pose-v1.md`](design/pose-3d-fusion-pose-v1.md)。
>
> **2026-08-16 更新 (D50 tracker-axis producer)**: FusionPoseのraw capture/lifecycleを
> position/quality値なしのcompact lineageとしてpost-filter snapshotまで保持し、既存
> `TrackerExtractor`のOne-Euro後quaternionからChest/Hips body-rightと脚4本の
> proximal-to-distal axisを生成する。raw source jointが同一captureで全て実測された場合だけ
> Freshとし、predict/hold/IK/FK fillはUnavailable。不明timestampはfail closed、通常frameは
> latest-only、境界はraw→tracker handoffとpublic deliveryの両方で有界FIFO + explicit
> continuity resetとする。`GET /api/tracker-axis` / `WS /ws/tracker-axis`をrun/calib-subjectへ
> 配線し、FusionPoseのclock-sync処理を共用する。詳細は
...
| pose-3d 顔点除外 | `ctest --test-dir cpp/build --output-on-failure -R 'triangulator\|kalman_chain\|pose_recognizer'` + 同一subject calibration録画の変更前後比較 | Halpe26 eyes/ears (1–4) は3D invalid。nose (0) はposition-only DLTから `head_top` 起点0.15m・頭軸直交のdirection endpointへ合成し、3成分EMAだけで平滑化する。`view_count/reproj=0`、6D Kalman/IK/profile/valid count対象外。body 21点と `head_top` / `neck` は有効。顔だけ大きくずらしても再投影中央値・IK bone driftが悪化せず、2D JSONは26点互換を維持。設計: [`design/pose-3d-face-joint-exclusion.md`](design/pose-3d-face-joint-exclusion.md) |
| pose-3d `fitra_pose_gate_v1` | `cmake --build cpp/build && ctest --test-dir cpp/build --output-on-failure -R 'sync_matcher\|pose_gate'` + 実機 `--enable-3d --keypoint-format=halpe26` で `WS /ws/pose-gate` を JSONL 保存 | `test_sync_matcher` が10–25ms位相差、カメラ停止時の一回限りの境界、復旧を pass。`test_pose_gate` が正常、関節欠損、再接続、人物切替、epoch変更、Halpe26以外、multi-person拒否、quaternion禁止、診断値を pass。実機は I/Ski 用8関節が `Fresh`、短い同期待ちで `Unavailable -> Reacquired` を反復せず、欠損/人物切替/再接続/再較正で Unavailable 境界と track ID/epoch 変更が記録され、`content_mono_ns` が単調時計であること。`/api/pose-gate` と `/stats3d.stats.pose_gate` の `sync_dt_ms` min/median/max も保存する。fixture: [`samples/fitra_pose_gate_v1`](samples/fitra_pose_gate_v1)、設計: [`design/pose-3d-fitra-pose-gate-v1.md`](design/pose-3d-fitra-pose-gate-v1.md) |
| pose-3d `fitra_fusion_pose_v1` | `ctest --test-dir cpp/build --output-on-failure -R 'fusion_pose\|pose_gate\|triangulator\|crow_excal'` + 実機 `WS /ws/fusion-pose` をJSONL保存 | `test_fusion_pose` がV4L2 monotonic SOE/EOF、unknown/mixed unavailable、HALPE26左右shoulderを含むexact 10 jointとnumber/null型、raw/filtered位置の分離、raw品質由来の`observed_this_frame`、Hips保守midpoint、predict-only/関節単位欠損、既存PoseGate exact 8/byte互換、latest/boundary順序、stream/subject/coordinate/continuity切断、overflow reset、numeric clock pongをpass。`test_triangulator`がmax acute ray angleをreprojection除外後の最終inlierだけから算出し、`test_crow_excal`がexact 10の実GET/WSとping/pongをpass。実機はkernel timestamp意味論、mixed/unknown fallback、capture span、filtered位置がfloor/root補正前であること、Fusion WS接続前後のcapture cadence不変を確認する。設計: [`design/pose-3d-fusion-pose-v1.md`](design/pose-3d-fusion-pose-v1.md) |
| pose-3d `fitra_tracker_axis_v1` | `ctest --test-dir cpp/build --output-on-failure -R 'pose_gate\|fusion_pose\|tracker_axis\|tracker_extract\|kalman\|crow_excal'` + 実機 GET/WS schema確認 | `test_tracker_axis`がSOE/EOF、axis符号/unit、raw source欠損によるpredict-only拒否、exact 6順序/nullable shape、境界順序、lineage/public queue overflow continuity、numeric clock pongをpass。`test_crow_excal`が実GET/WSとping/pongをpass。実機は通常mainのFresh exact6、clock-sync、legacy PoseGate exact8/FusionPose exact10、Windows GET非404を値なし確認する。設計: [`design/pose-3d-tracker-axis-v1.md`](design/pose-3d-tracker-axis-v1.md) |
...
| pose-3d floor-contact stability | `ctest --test-dir cpp/build --output-on-failure -R 'floor_contact_stabilizer\|main_config\|tracker_extract\|kalman_chain'` + `dump_keypoints_3d --fps <measured>` を同一静止録画へ既定 ON / `--no-floor-contact-stability` で A/B + 実機 WebUI / VMT / SlimeVR | 左右独立・剛体平行移動・XY jitter 80%以上減衰・孤立 sole 外れ値除外・接地中Z補正の正負上限・8cm 超貫通の有界 fail-safe・単発離地候補を2回+50ms graceで吸収・継続離地と補正単調減衰・grace/実適用/fresh 分離・極小dt速度計算・8fps latch・FootAnchor 補正前脚長・COCO17 no-op が pass。接地率は current evidence を分母にして ≤1。静止 A/B は `ankle_xy_rms_m_pooled` が OFF 比40%以上低下、ON の `sole_below_floor_fraction == 0`、補正ノルム p95 ≤ 0.089m。実機は接地リングと `/stats3d` が一致し、sync miss で air 点滅せず、静止で足が滑らず、歩行/足踏みで離地遅延・snap がない。問題時は `--no-floor-contact-stability` で従来経路へ戻る。設計: [`design/pose-3d-floor-contact-stability.md`](design/pose-3d-floor-contact-stability.md) |
| pose-3d 四肢伸展 | `ctest --test-dir cpp/build --output-on-failure -R 'tracker_extract\|firmware_protocol\|vmt_protocol\|main_config'` + 製品既定 ON と `--no-limb-extension-snap` / `--no-extended-leg-toe-direction` を組み合わせた 4 条件の実機 A/B | `MainOptions` / `TrackerExtractorOptions` は両機能 ON、低レベル比較 baseline は OFF で完全一致。private skeleton 上の腕/脚 snap が 2 ボーン長を保存し、元 Skeleton3D は不変。伸展方向 20° enter / 屈曲方向 12° exit の逆向き hysteresis は連続 2 frame の方向確認を行い、欠損・1-frame spike・中間帯反転で chatter しない。toe 有効時は thigh/shin twist が同じ足先基準へ連続、toe 欠損時は held roll へ fallback、屈曲脚は旧出力と一致。実機は立位/T-pose/腕前方伸展/足先内外旋/歩行/しゃがみで誤 snap・180° roll flip が増えないこと。設計: [`design/pose-3d-limb-extension-snap.md`](design/pose-3d-limb-extension-snap.md) |
