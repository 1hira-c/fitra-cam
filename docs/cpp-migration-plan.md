...
> 8関節の最終inlier品質、capture区間、source publish monotonic時刻、continuityを公開し、通常poseは
...
| pose-3d `fitra_fusion_pose_v1` | `ctest --test-dir cpp/build --output-on-failure -R 'fusion_pose\|pose_gate\|triangulator\|crow_excal'` + 実機 `WS /ws/fusion-pose` をJSONL保存 | `test_fusion_pose` がV4L2 monotonic SOE/EOF、unknown/mixed unavailable、exact 8 jointとnumber/null型、既存PoseGate byte互換、latest/boundary順序、stream/subject/coordinate/continuity切断、overflow reset、numeric clock pongをpass。`test_triangulator`がmax acute ray angleをreprojection除外後の最終inlierだけから算出し、`test_crow_excal`が実WS ping/pongをpass。実機はkernel timestamp意味論、mixed/unknown fallback、capture span、Fusion WS接続前後のcapture cadence不変を確認する。設計: [`design/pose-3d-fusion-pose-v1.md`](design/pose-3d-fusion-pose-v1.md) |
