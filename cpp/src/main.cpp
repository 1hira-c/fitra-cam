...
void print_help() {
    std::puts(
        ...
        "  --st-filter / --no-st-filter\n"
        "                            spatiotemporal tracker filter (default ON since M-C5;\n"
        "                            --no-st-filter falls back to One Euro)\n"
        "  --rigid-pelvis            spatial pelvis rigid fit (needs subject profile + halpe26)\n"
        "  --floor-grounding         ground foot sole points to the floor plane (halpe26)\n"
        "  --roll-hysteresis         arm/thigh roll gate-raise hysteresis (default off)\n"
        "  --vr-extract-event-driven react to each 3D frame (default on)\n"
        "  --no-vr-extract-event-driven\n"
        "                            use fixed-rate tracker extraction for A/B\n"
