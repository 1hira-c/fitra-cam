...
//
// Phase 9: `schema` distinguishes COCO17 (`fitra_subject_profile_v1`) from
// Halpe26 (`fitra_subject_profile_v2`). Loading a v1 profile while running
// in Halpe26 mode (or vice versa) raises -- profiles are not migrated.
...
struct SubjectProfile {
    ...
    // calibrations on disk; the Phase 8 wizard writes v2 when running under
...
// active keypoint format (used by the Phase 8 analysis pipeline).
