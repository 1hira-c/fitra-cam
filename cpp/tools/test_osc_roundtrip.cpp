// test_osc_roundtrip — exercise OscWriter against hand-crafted byte tables.
//
...
// short of actually running SlimeVR.
...
// Helpers to append common OSC fragments to a "want" buffer.
...
void test_message_with_ints() {
    // /foo ,iiii 1000 -1 0 0x7FFFFFFF
    fitra::slimevr::OscWriter w;
    w.clear();
    w.begin_message("/foo");
    w.add_int(1000);
    w.add_int(-1);
...
void test_bundle_two_messages() {
    // #bundle, ts, [msg /a ,i 7], [msg /bb ,f 1.5]
    fitra::slimevr::OscWriter w;
    w.clear();
...
void test_vmc_tra_pos_packet_shape() {
    // The actual payload shape we'll send to SlimeVR. This is the highest-
    // value regression target -- if this drifts, SlimeVR drops the packet.
    // /VMC/Ext/Tra/Pos ,sfffffff "waist" px py pz qx qy qz qw
    fitra::slimevr::OscWriter w;
    w.clear();
    w.begin_bundle(0);
    ...
    w.add_float(0.0f); w.add_float(0.0f);  w.add_float(0.0f); w.add_float(1.0f);
    w.end_message();
    w.end_bundle();
    ...
    auto got = w.data();
    check_bytes(got.data(), got.size(), want.data(), want.size(), "vmc_tra_pos_packet_shape");
}
...
int main() {
    try {
        ...
        test_empty_bundle();
        test_vmc_tra_pos_packet_shape();
        std::puts("test_osc_roundtrip ok");
        return 0;
    } catch (const std::exception& e) {
        std::fprintf(stderr, "test_osc_roundtrip failed: %s\n", e.what());
        return 1;
    }
}
